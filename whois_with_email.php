...
<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <br>
            <b>
                Please enter the host to be looked up: 
            </b>
            ...
            <br>
            <b>
                E-Mail Address:  &nbsp &nbsp &nbsp &nbsp
            </b>
            <input type=text id='email' name='email' value=''>
            <br>
			(Kindly enter the email address here to recieve the results by email)
            <br>
            <br>
        </form>
        <?php
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $valid = validate_host($host);
    $email_valid = validate_email($email_to);
    switch ($valid){
        case 'FQDN':
        $result=FQDNLookup($host);
        If($email_valid){ sendemail($email_to,$result); }
        echo "<pre>\n" . $result . "\n</pre>\n";
        break;
        case 'IP':
        $result=IPLookup($host);
        If($email_valid){ sendemail($email_to,$result); }
        echo "<pre>\n" . $result . "\n</pre>\n";
        break;
        case 'INVALID':
...
function validate_host($hostv) {
    if(preg_match("/([-a-z0-9]{2,100})\.([a-z\.]{2,8})$/i", $hostv)) {
        return 'FQDN';
    }
    $ipnums = explode(".", $hostv);
    if(count($ipnums) == 4) {
        foreach($ipnums as $ipnum) {
            $tmp='INVALID';
            if(is_numeric($ipnum) and ($ipnum < 256)) {
                $tmp='IP';
            }
        }
        return $tmp; 
...
function sendemail($to,$res){
    $msg = wordwrap($res,70);
    $headers = "MIME-Version: 1.0" . "\r\n";
    $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
    $headers .= "From: bala@balasphp.com" . "\r\n" ;
    mail($to,"Whois_Lookup_results",$msg,$headers);
}
        ?>
    </body>
</html>